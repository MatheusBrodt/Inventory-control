...
# ==========================//====================/LIMPEZA DE ENTRYS/====================//=============================
    ...
    def clear_dadosRegServiço(self):  # LIMPA OS DADOS DO REGISTRO DE SERVIÇOS
        if self.verif:
            ...
            self.shopNove_seqRem = self.shopNove_seq.delete(0, END)
    ...
    def clear_MovService(self):
        self.boxEntryRem = self.box.delete(0, END)
        self.cod_shopEnrtyRem = self.cod_Shop.delete(0, END)
...
# ==========================//====================/CAPTURA DE DADOS/====================//=============================
    ...
    def captura_dadosRegServico(self):  # CAPTURA OS DADOS DAS ENTRYS DO REGISTRO DE SERVIÇOS
        ...
        self.shopNove_seqEntryCapt = str(self.shopNove_seq.get()).strip()
    ...
    def captura_ListService(self, event=''):  # CAPTURA AS ENTRYS DA LISTA DE SERVIÇO
        verif = []
...
# ==========================//=====================/MANIPULAÇÃO DE DADOS/==================//===========================
            ...
            if data_prev >= data_today:
                ...
                if '-G' in garantia or '-g' in garantia:
                    ...
                    self.cursor.execute(f"INSERT INTO services VALUES "
                                        ...
                                        f"'{previsao}', '', 'Sim', '', '{self.shopNove_seqEntryCapt}', '')")
                    self.conn.commit()
                    ...
                    if exist is None:
                        ...
                        self.cursor.execute(f"INSERT INTO services VALUES "
                                            ...
                                            f"'{self.store_RegServiceEntryCapt}', '{self.seq_RegServiceEntryCapt}', "
                                            f"'{self.tipo_RegServiceEntryCapt}', '{self.sit_RegServiceEntryCapt}', "
                                            f"'{previsao}', '', DEFAULT, '', '{self.shopNove_seqEntryCapt}', '')")
                        self.conn.commit()
                        self.text_warning = 'SERVIÇO ADICIONADO'
                        self.warning()
                        self.clear_dadosRegServiço()
    ...
    def pesq_Service(self):
        self.connect_BD()
        self.cursor.execute(f"SELECT store, sequencia, data, tipo, situation, previsao, obs, box, status FROM services "
                            f"WHERE LOCATE('{self.seq_PesqCapt}', sequencia)")
        lista = self.cursor.fetchall()
        if lista == []:
    ...
    def insert_Obs(self):
        ...
        for dado in self.listaPesq.selection(): # INSERIR DADOS NAS ENTRYS DA OBS
            col1, col2, col3, col4, col5, col6, col7, col8, col9 = self.listaPesq.item(dado, 'values')
            self.col1 = col1
            self.col2 = col2
            self.col4 = col4
    ...
    def createDBLocal(self):
        try:
            banco = sqlite3.connect('local.db')
            cursor = banco.cursor()
            cursor.execute(f'CREATE TABLE IF NOT EXISTS services ('
                                  f'box BIGINT,'
                                  f'cod_shop VARCHAR(20))')
            banco.commit()
            banco.close()
            print('Banco e tabelas construídos!')
        except:
            print('Banco já foi criado!')

    def add_Row(self, event=''):  # Adicionar a fila de espera de montagem, banco de dados local
        serv = []
        box = self.box.get()
        cod_shop = self.cod_Shop.get()
        serv.append(box)
        serv.append(cod_shop)
        if box == '' or cod_shop == '':
            self.text_warning = 'PREENCHA TODOS OS CAMPOS'
            self.warning()
        else:
            self.lista_Mov.insert('', END, values=(serv))
            self.text_warning = ''
            self.warning()
            bd = sqlite3.connect('local.db')
            cursor = bd.cursor()
            cursor.execute(f'INSERT INTO services VALUES ("{serv[0]}", "{serv[1]}")')
            bd.commit()
            serv = []
            self.clear_MovService()
            cursor.execute(f'SELECT * FROM services')
            result = cursor.fetchall()


    def add_rowBD(self):
        self.connect_BD() # BANCO DE DADOS NA NUVEM
        ...
        result = cur.fetchall()
        if result == []:
            self.text_warning = 'SEM SERVIÇOS P/ ADICIONAR'
            self.warning()
        else:
            self.text_warning = ''
            self.warning()
            for serv in result:
                self.cursor.execute(f"SELECT situation FROM services WHERE cod_shop9 = '{serv[1]}'")
                teste = self.cursor.fetchone()
                if teste is not None:
                    if teste[0] == 'Aguardando' or teste[0] != 'Retrabalho' or teste[0] != 'Montagem':
                        self.cursor.execute(f"UPDATE services SET situation = 'Lentes Separadas', box = '{serv[0]}' "
                                            f"WHERE cod_shop9 = '{serv[1]}' LIMIT 1")
                        self.conn.commit()
                        cur.execute(f'DELETE FROM services WHERE cod_shop = "{serv[1]}"')
                        bd.commit()
                    if teste[0] != 'Aguardando' and teste[0] != 'Retrabalho'and teste[0] != 'Montagem':
                        if teste[0] == 'Lentes Separadas':
                            warn = 'Lentes já separadas'
                            self.lista_NotMov.insert('', END, values=(serv[0], serv[1], warn))
                        else:
                            warn = 'Rota Errada'
                            self.lista_NotMov.insert('', END, values=(serv[0], serv[1], warn))
                else:
    ...
    def add_montBD(self):
        ...
        if result == []:
            ...
            for serv in result:
                ...
                teste = self.cursor.fetchone()
                if teste is not None:
                    if teste[0] == 'Aguardando' or teste[0] == 'Lentes Separadas' or teste[0] == 'Retrabalho':
                        self.cursor.execute(f"UPDATE services SET situation = 'Montagem', box = '{serv[0]}' "
                                            f"WHERE cod_shop9 = '{serv[1]}' LIMIT 1")
                        self.conn.commit()
                        cur.execute(f'DELETE FROM services WHERE cod_shop = "{serv[1]}"')
                        bd.commit()
                    if teste[0] != 'Aguardando' and teste[0] != 'Lentes Separadas' and teste[0] != 'Retrabalho':
                        if teste[0] == 'Montagem':
                            warn = 'Está na Montagem'
                            self.lista_NotMov.insert('', END, values=(serv[0], serv[1], warn))
                        else:
                            warn = 'Rota Errada'
                            self.lista_NotMov.insert('', END, values=(serv[0], serv[1], warn))
    ...
    def urgent_service(self):
        self.connect_BD()
        self.date_Today()
        self.date
        self.cursor.execute(f"SELECT store, sequencia, previsao, tipo, warrant FROM services WHERE "
                            f"previsao = '{self.date}' AND situation != 'Finalizado' "
                            f"OR situation = 'Urgente' ORDER BY store DESC")
        lista = self.cursor.fetchall()
...
# ==========================//============================/PDF/============================//===========================
    ...
    def label_RegService(self):
        ...
        shopNoveSeq = Label(self.frame_options, text='Cód. Shop 9:', font=self.fontepadrao, bg='#F0E68C')
        ...
        self.shopNove_seq = Entry(self.frame_options, font=self.fontepadrao, bg='white')
        self.shopNove_seq.bind("<Return>", self.option_RegServiceEnter)
        self.sit_RegServiceEntry = ttk.Combobox(self.frame_options, font=self.fontepadrao)
        self.sit_RegServiceEntry['values'] = ('Digitação', 'Aguardando', 'Montagem', 'Retrabalho', 'Urgente',
                                              'Montagem Farol', 'Montagem Forla', 'Montagem Wallau', 'Montagem Paris',
                                              'Montagem zeiss')
        ...
        shopNoveSeq.place(relx=0.28, rely=0.10, relwidth=0.17, relheight=0.045)
        self.tipo_RegService.place(relx=0.63, rely=0.03, relwidth=0.12, relheight=0.045)
        self.sit_RegService.place(relx=0.06, rely=0.185, relwidth=0.12, relheight=0.045)
        self.prev_RegService.place(relx=0.45, rely=0.185, relwidth=0.12, relheight=0.045)
        self.prevDay_RegService.place(relx=0.57, rely=0.14, relwidth=0.10, relheight=0.045)
        self.prevMonth_RegService.place(relx=0.705, rely=0.14, relwidth=0.10, relheight=0.045)
        self.prevYear_RegService.place(relx=0.84, rely=0.14, relwidth=0.10, relheight=0.045)
        #  LOCALIZAÇÃO DAS ENTRYS
        ...
        self.shopNove_seq.place(relx=0.45, rely=0.10, relwidth=0.30, relheight=0.045)
        self.tipo_RegServiceEntry.place(relx=0.73, rely=0.03, relwidth=0.21, relheight=0.045)
        self.sit_RegServiceEntry.place(relx=0.185, rely=0.185, relwidth=0.20, relheight=0.045)
        self.prevDay_RegServiceEntry.place(relx=0.58, rely=0.185, relwidth=0.08, relheight=0.045)
        self.prevMonth_RegServiceEntry.place(relx=0.663, rely=0.185, relwidth=0.175, relheight=0.045)
        self.prevYear_RegServiceEntry.place(relx=0.84, rely=0.185, relwidth=0.10, relheight=0.045)

    def label_services(self):
        # LABELS
        self.fontepadrao = ("Verdana", 10, "italic", 'bold')
        fonte_text = ("Verdana", 18, 'bold')
        ...
        text = Label(self.frame_services, text='Serviços Do Dia', font=fonte_text, bg='#FFD700')
        # ENTRYS
        ...
        self.store_serviceEntry['values'] = ('2064', '1432', '2007', '1518', '1571', '1744', '1574', '1648', '2226',
                                             '2322', '2368')
        self.store_serviceEntry.bind("<Return>", self.captura_ListService)
        self.seq_serviceEntry = Entry(self.frame_services, font=self.fontepadrao, bg='white')
        self.seq_serviceEntry.bind("<Return>", self.captura_ListService)
        self.type_serviceEntry = Entry(self.frame_services, font=self.fontepadrao, bg='white')
        self.type_serviceEntry.bind("<Return>", self.captura_ListService)
        # LOCALIZAÇÃO DAS LABELS
        ...
        text.place(relx=0.01, rely=0.09, relwidth=0.98, relheight=0.08)
        # LOCALIZAÇÃO DAS ENTRYS
    ...
    def label_Pesq(self):
        self.fontepadrao = ("Verdana", 10, "italic", 'bold')
        # LABELS
        self.cod_ShopNove = Label(self.frame_options, text='Cód. Shop 9:', font=self.fontepadrao, bg='#f0e68c')
        self.seqServPesq = Label(self.frame_options, text='Sequência:', font=self.fontepadrao, bg='#f0e68c')
        # ENTRYS
        self.cod_ShopNoveEntry = Entry(self.frame_options, font=self.fontepadrao)
        self.cod_ShopNoveEntry.bind("<Return>", self.option_buttonPesqEnter)
        ...
        # LOCALIZAÇÃO DAS LABELS
        self.cod_ShopNove.place(relx=0.065, rely=0.07, relwidth=0.17, relheight=0.045)
        self.seqServPesq.place(relx=0.56, rely=0.07, relwidth=0.15, relheight=0.045)
        # LOCALIZAÇÃO DAS ENTRYS
        self.cod_ShopNoveEntry.place(relx=0.235, rely=0.07, relwidth=0.22, relheight=0.045)
        self.seq_PesqEntry.place(relx=0.71, rely=0.07, relwidth=0.22, relheight=0.045)
    ...
    def labelAlter_Status(self):
        ...
        self.situationEntry['values'] = ('Digitação', 'Aguardando', 'Montagem', 'Retrabalho', 'Urgente',
                                          'Montagem Farol', 'Montagem Forla', 'Montagem Wallau', 'Montagem Paris',
                                          'Montagem zeiss')
    ...
    def frame_Mov(self):
        self.fontepadrao = ("Verdana", 10, "italic", 'bold')
        # LABELS
        box = Label(self.frame_options, text='Caixa:', bg='#f0e68c', fg='black', font=self.fontepadrao)
        cod_Shop = Label(self.frame_options, text='Cód. Shop 9:', bg='#f0e68c', fg='black', font=self.fontepadrao)
        add_serv = Label(self.frame_options, text='Serviços para adicionar!', bg='#F0E68C', fg='black',
                         font=self.fontepadrao)
        not_addServ = Label(self.frame_options, text='Serviços não adicionados!', bg='#F0E68C', fg='red',
                            font=self.fontepadrao)
        # ENTRYS
        self.box = Entry(self.frame_options, font=self.fontepadrao, bg='white')
        self.cod_Shop = Entry(self.frame_options, font=self.fontepadrao, bg='white')
        self.cod_Shop.bind("<Return>", self.add_Row)
        # LOCALIZAÇÃO
        box.place(relx=0.10, rely=0.03, relwidth=0.12, relheight=0.045)
        cod_Shop.place(relx=0.40, rely=0.03, relwidth=0.17, relheight=0.045)
        add_serv.place(relx=0.08, rely=0.10, relwidth=0.35, relheight=0.045)
        not_addServ.place(relx=0.57, rely=0.10, relwidth=0.35, relheight=0.045)
        self.box.place(relx=0.21, rely=0.03, relwidth=0.12, relheight=0.045)
        self.cod_Shop.place(relx=0.575, rely=0.03, relwidth=0.27, relheight=0.045)
...
# ==========================//===========================/LISTAS/========================//=============================
    ...
    def lista_searchService(self, event=''):
        ...
        self.listSearchServ.heading('col3', text='Previsão')
        self.listSearchServ.heading('col4', text='Tipo')
        self.listSearchServ.heading('col5', text='Garantia')

        self.listSearchServ.column('#0', width=0)
        self.listSearchServ.column('col1', width=50)
        self.listSearchServ.column('col2', width=70)
        self.listSearchServ.column('col3', width=90)
        self.listSearchServ.column('col4', width=90)
        self.listSearchServ.column('col5', width=50)

        self.listSearchServ.place(relx=0.025, rely=0.17, relwidth=0.948, relheight=0.77)
    ...
    def lista_Pesq(self):
        ...
        self.listaPesq.heading('col8', text='Caixa')
        self.listaPesq.heading('col9', text='Status')
        ...
        self.listaPesq.column('col9', width=20)

        self.listaPesq.place(relx=0.025, rely=0.18, relwidth=0.95, relheight=0.65)
        self.listaPesq.bind('<Double-1>', self.Obs)

    def list_MovServ(self):
        self.lista_Mov = ttk.Treeview(self.frame_options, height=3, columns=('col1', 'col2', 'col3'))
        self.lista_Mov.heading('#0')
        self.lista_Mov.heading('col1', text='Caixa')
        self.lista_Mov.heading('col2', text='Cód. Shop 9')

        self.lista_Mov.column('#0', width=0)
        self.lista_Mov.column('col1', width=80)
        self.lista_Mov.column('col2', width=200)

        self.lista_Mov.place(relx=0.02, rely=0.18, relwidth=0.47, relheight=0.65)

    def list_NotMovServ(self):
        self.lista_NotMov = ttk.Treeview(self.frame_options, height=3, columns=('col1', 'col2', 'col3'))
        self.lista_NotMov.heading('#0')
        self.lista_NotMov.heading('col1', text='Caixa')
        self.lista_NotMov.heading('col2', text='Cód. Shop 9')
        self.lista_NotMov.heading('col3', text='Motivo')

        self.lista_NotMov.column('#0', width=0)
        self.lista_NotMov.column('col1', width=50)
        self.lista_NotMov.column('col2', width=100)
        self.lista_NotMov.column('col3', width=100)

        self.lista_NotMov.place(relx=0.51, rely=0.18, relwidth=0.47, relheight=0.65)
...
# =========================//=========================/BOTÕES ENTER/========================//==========================
    ...
    def button_Selecionar(self):
        self.fontepadrao = ("Verdana", 10, "italic", 'bold')
        self.AddService = Button(self.frame_services, text='Recarregar', font=self.fontepadrao, bg='#f0e68c',
        ...
        self.AddService.place(relx=0.41, rely=0.947, relwidth=0.18, relheight=0.04)
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    #BOTÃO ENTER DE ADD BOX
    def button_EnterBox(self):
        self.EnterAddBox = Button(self.frame_options, font=self.fontepadrao, text='Add. Fila', bg='#f0e68c',
                               command=self.option_EnterBox)
        self.EnterAddBox.bind('<Enter>', self.passou_por_cima)
        self.EnterAddBox.bind('<Leave>', self.saiu_de_cima)
        self.EnterAddBox.place(relx=0.15, rely=0.92, relwidth=0.22, relheight=0.05)
    def option_EnterBox(self, event=''):
        print('Botão ENTER Add Box clicado!')
        self.add_rowBD()
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    #BOTÃO ENTER DE ADICIONAR NA MONTAGEM
    def button_EnterLens(self):
        self.EnterAddLens = Button(self.frame_options, font=self.fontepadrao, text='Add. Montagem', bg='#f0e68c',
                               command=self.option_EnterLens)
        self.EnterAddLens.bind('<Enter>', self.passou_por_cima)
        self.EnterAddLens.bind('<Leave>', self.saiu_de_cima)
        self.EnterAddLens.place(relx=0.62, rely=0.92, relwidth=0.22, relheight=0.05)
    def option_EnterLens(self, event=''):
        print('Botão ENTER Add Lens clicado!')
        self.add_montBD()
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
# =========================//============================/BOTÕES/===========================//==========================
    ...
    def option_RegisterLens(self, event=''):
        ...
        bd = sqlite3.connect('local.db')
        cur = bd.cursor()
        cur.execute(f'DELETE FROM services')
        bd.commit()
        cur.close()
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def button_MovServ(self):
        self.buttonMovService = Button(self.frame_buttons, text='Mov. Serv.', bg='#c0c0c0', fg='black',
                                       command=self.option_ButtonMov)
        self.buttonMovService["font"] = ("Verdana", 10, "italic", "bold")
        self.buttonMovService.bind("<Enter>", self.passou_por_cima)
        self.buttonMovService.bind("<Leave>", self.saiu_de_cima)
        self.buttonMovService.place(relx=0.05, rely=0.64, relwidth=0.90, relheight=0.05)

    def option_ButtonMov(self):
        print('Botão de movimentação de serviços clicado!')
        self.options()
        self.frame_Mov()
        self.button_EnterBox()
        self.button_EnterLens()
        self.list_MovServ()
        self.list_NotMovServ()
        bd = sqlite3.connect('local.db')
        cur = bd.cursor()
        cur.execute(f'DELETE FROM services')
        bd.commit()
